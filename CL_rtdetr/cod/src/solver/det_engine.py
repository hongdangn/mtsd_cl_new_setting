...
def fake_query(outputs, targets, class_ids, topk=30, threshold=0.3):
    ...
    return targets
#abc
def get_teacher_output(outputs, class_ids,topk=30):
    out_logits, out_bbox = outputs["pred_logits"], outputs["pred_boxes"]
...
def train_one_epoch(
    ...
    **kwargs,
    ...
    for batch_idx, (samples, targets) in enumerate(tqdm_batch):
        ...
        if distill_attn:
            ###########abc
            #teacher_outputs1 = teacher_model(samples,targets)
            #print("teacher_outputs1",teacher_outputs1)
            
            teacher_outputs2 = teacher_model(samples)
            #print("teacher_outputs2",teacher_outputs2)
            
            teacher_targets=get_teacher_output(teacher_outputs2, divided_classes[task_idx])

            student_outputs=model(samples,targets)
            
            #print("teacher_targets",teacher_targets)
            newtarget = [{'labels': label, 'score': score.item(), 'box': [0, 0, 1, 1]} for t in teacher_targets for label, score in zip(t['labels'], t['scores'])]

            distill_loss=criterion(student_outputs,newtarget)
